...
struct PipeGemmBiasLayerNorm {
    ...
    struct SmemBuffers {
        ...
        half gamma[ProblemSize::kN];
        half beta[ProblemSize::kN];
        half ln_sbuf[2 * ProblemSize::kN];
        float shared_mean;
        float shared_var;
    };
};
...
__device__ void pipe_gemm_bias_layer_norm(
    ...
    using Types = PipeGemmBiasLayerNorm<ProblemShape>;
    ...
    for (size_t i = 0; i < num_iters; i++) {
        ...
        internal_layer_norm<ProblemShape::kN>(
            o_ptr,
            &smem->gamma[0],
            &smem->beta[0],
            o_ptr,
