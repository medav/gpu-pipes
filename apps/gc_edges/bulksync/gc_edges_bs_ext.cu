...
void gc_edges_out(
    ...
    assert(x.size(0) == MM);
    assert(x.size(1) == 1536);
    assert(w0.size(0) == 1536 && w0.size(1) == 512 && b0.size(0) == 512);
...
at::Tensor gc_edges(
    ...
    gc_edges_out(x, w0, b0, w1, b1, ga0, be0, t0, t1, out);
    return out;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("gc_edges", &gc_edges, "gc_edges");
    m.def("gc_edges_out", &gc_edges_out, "gc_edges_out");
}
