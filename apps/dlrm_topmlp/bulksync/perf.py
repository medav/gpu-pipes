...
def test_performance(NI=1000):
    ...
    w5 = torch.randn(256, 32, dtype=torch.float16, device='cuda')
    b5 = torch.randn(32,      dtype=torch.float16, device='cuda')

    t1 = torch.zeros(M, 1024, dtype=torch.float16, device='cuda')
    t2 = torch.zeros(M, 1024, dtype=torch.float16, device='cuda')
    t3 = torch.zeros(M, 512, dtype=torch.float16, device='cuda')
    t4 = torch.zeros(M, 256, dtype=torch.float16, device='cuda')
    out = torch.zeros(M, 32, dtype=torch.float16, device='cuda')
