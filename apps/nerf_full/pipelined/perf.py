...
def test_performance(NI=1000):
    ...
    w9, b9 = linear_weights(256, 64)

    w10, b10 = linear_weights(256, 256)
    w11, b11 = linear_weights(288, 128)
    w12, b12 = linear_weights(128, 64)

    out_r = torch.zeros(M, 64, dtype=torch.float16, device='cuda')
    out_rgb = torch.zeros(M, 64, dtype=torch.float16, device='cuda')
